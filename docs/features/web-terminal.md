...
- `GET /projects/{slug}/terminal?agent=cursor|opencode` — terminal page (auth required).
- `WS /ws/terminal/{slug}/{agent}` — bidirectional PTY to `tmux attach-session` after zooming the target pane (`0` = cursor, `1` = opencode).
- Attach by **project slug** + agent (not short repo name).
